...
const SkeletonOverlay = ({ onPartClick }) => {
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto">
      <img src={skeleton} alt="skeleton" className="w-full h-auto object-contain" />
        ...
        {Object.entries(bodyParts).map(([limb, parts]) =>
          parts.map((part, index) => (
            <button
              ...
              title={part.name}
            >
              <span className="hidden sm:inline-block text-[10px] sm:text-xs text-white bg-blue-600/60 rounded px-1 py-0.5">
                {part.name}
              </span>
            </button>
          ))
