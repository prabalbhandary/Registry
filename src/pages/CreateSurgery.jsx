...
import { FaPlus, FaHospital, FaArrowRight } from "react-icons/fa6";
...
const CreateSurgery = () => {
  ...
  // ── Step state ──────────────────────────────────────────────────────────────
  const [step, setStep] = useState("hospital"); // "hospital" | "form"
  ...
  const [hospitalModalLoading, setHospitalModalLoading] = useState(false);
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (nepal) setCountryId(nepal.id);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    if (isNepal) {
      axios
        .get(`${BASE_URL}/provinces?country_id=${country_id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
        .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE_URL}/districts?province_id=${province_id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
      .then((res) => {
      ...
      .catch((err) => {
        if (err.response?.status === 401) {
          ...
          toast.error(err.response?.data?.message || "Failed to fetch districts");
        }
      });
  }, [province_id]);
  ...
  const handleAddHospital = async () => {
    ...
    try {
      ...
      if (newHospital) {
        const newOption = { value: newHospital.id, label: newHospital.name };
        setHospitals((prev) => [...prev, newOption]);
        setSelectedHospital(newOption);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name) { formValid = false; newErrors.name = "Name is required."; }
    ...
    if (isNepal) {
      ...
      if (!district_id) { formValid = false; newErrors.district_id = "District is required."; }
    } else {
      if (!address) { formValid = false; newErrors.address = "Address is required."; }
    }

    if (!hospital_number) { formValid = false; newErrors.hospital_number = "Hospital number is required."; }
    if (!phone_number) { formValid = false; newErrors.phone_number = "Phone number is required."; }
    if (!occupation) { formValid = false; newErrors.occupation = "Occupation is required."; }
    if (!mechanism_of_injury) { formValid = false; newErrors.mechanism_of_injury = "Mechanism of Injury is required."; }
    if (!type_of_injury && mechanism_of_injury !== "others") { formValid = false; newErrors.type_of_injury = "Sub-option is required."; }
    if (!incidentDate) { formValid = false; newErrors.incidentDate = "Incident Date is required."; }
    if (!incidentTime) { formValid = false; newErrors.incidentTime = "Incident Time is required."; }
    if (!primaryTreatment) { formValid = false; newErrors.primaryTreatment = "Primary Treatment is required."; }

    if (!formValid) { setFieldErrors(newErrors); return; }
    ...
    try {
      const res = await axios.post(
        `${URL}/patient-detail`,
        {
          ...
          hospital_number, phone_number, occupation,
          mechanism_of_injury, type_of_injury, sportsOther, MoIOther,
  ...
  const handleSubMoIChange = (e) => setType_of_Injury(e.target.value);
  ...
  const customSelectStyles = {
    control: (base, state) => ({
      ...
      "&:hover": { borderColor: state.isFocused ? "#4f46e5" : "#d1d5db" },
      borderRadius: "0.5rem",
      fontSize: "0.9375rem",
      transition: "all 0.2s",
    }),
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-8">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-indigo-700">Select Hospital</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-400">Patient Information</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <div className="flex gap-3 mb-8">
              <div className="flex-1">
                <Select
                  ...
                  isSearchable
                  noOptionsMessage={() => "No hospitals found"}
                />
              </div>
              <button
                type="button"
                ...
                className="p-3 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 text-white hover:shadow-lg transition-all"
              >
                <FaPlus />
              </button>
            </div>
            ...
            {selectedHospital && (
              <div className="mb-6 p-4 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center gap-3">
                <FaHospital className="text-indigo-500 text-lg shrink-0" />
                <div>
                  <p className="text-xs text-indigo-500 font-medium">Selected Hospital</p>
                  <p className="text-indigo-800 font-semibold">{selectedHospital.label}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between pt-4 border-t border-gray-100">
              <button
                type="button"
                onClick={() => navigate(-1)}
                className="px-6 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold hover:bg-gray-50 transition-all"
              >
                Cancel
              </button>
              <button
                ...
                className="px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-indigo-700 hover:shadow-xl transition-all flex items-center gap-2"
              >
        ...
        {isHospitalModalOpen && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl mx-4">
              <h2 className="text-xl font-bold text-gray-900 mb-1">Add New Hospital</h2>
              <p className="text-sm text-gray-500 mb-5">Fill in the details to add a new hospital</p>

              <label className="block text-sm font-semibold text-gray-700 mb-1">Hospital Name <span className="text-red-500">*</span></label>
              <input
                className="w-full p-3 border-2 border-gray-200 rounded-xl mb-3 focus:outline-none focus:border-indigo-400 transition-all"
              ...
              <input
                className="w-full p-3 border-2 border-gray-200 rounded-xl mb-5 focus:outline-none focus:border-indigo-400 transition-all"
              ...
              <div className="flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => { setIsHospitalModalOpen(false); setHospitalName(""); setHospitalAddress(""); }}
                  className="px-5 py-2.5 font-semibold text-gray-600 hover:bg-gray-50 rounded-xl transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={hospitalModalLoading}
                  className="px-6 py-2.5 rounded-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-indigo-700 hover:shadow-lg transition-all disabled:opacity-60"
                >
                  {hospitalModalLoading ? "Saving..." : "Save Hospital"}
                </button>
              </div>
            </div>
          </div>
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-8 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-8">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-indigo-200 text-indigo-700 flex items-center justify-center text-sm font-bold">✓</div>
              <span className="text-sm font-medium text-indigo-500">Select Hospital</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-indigo-700">Patient Information</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Add Patient Information</h1>
                  <p className="text-gray-500 text-sm">Please fill in the patient details to proceed</p>
                </div>
              </div>
              {/* Selected hospital badge */}
              <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2">
                <FaHospital className="text-indigo-500 text-sm" />
                <div>
                  <p className="text-xs text-indigo-400 font-medium">Hospital</p>
                  <p className="text-sm font-semibold text-indigo-800">{selectedHospital?.label}</p>
                </div>
                <button
                  type="button"
                  onClick={() => setStep("hospital")}
                  className="ml-2 text-xs text-indigo-500 underline hover:text-indigo-700"
                >
                  Change
                </button>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="text" id="fullName" name="name" value={name}
                    ...
                    />
                    {fieldErrors.name && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.name}</p>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="number" id="age" name="age" value={age}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    />
                    {fieldErrors.country_id && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.country_id}</p>}
                  </div>
                  ...
                  {isNepal && (
                    <div className="flex flex-col">
                      ...
                      <select
                        id="province" name="province" value={province_id}
                      ...
                      {fieldErrors.province_id && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.province_id}</p>}
                    </div>
                  )}
                  ...
                  {isNepal && (
                    <div className="flex flex-col">
                      ...
                      <select
                        id="district" name="district" value={district_id}
                      ...
                      {fieldErrors.district_id && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.district_id}</p>}
                    </div>
                  )}
                  ...
                  {country_id && !isNepal && (
                    <div className="flex flex-col md:col-span-2">
                      ...
                      />
                      {fieldErrors.address && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.address}</p>}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="text" id="hospital_number" name="hospital_number" value={hospital_number}
                    ...
                    />
                    {fieldErrors.hospital_number && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.hospital_number}</p>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="text" id="contact_number" name="phone_number" value={phone_number}
                    ...
                    />
                    {fieldErrors.phone_number && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.phone_number}</p>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    {fieldErrors.occupation && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.occupation}</p>}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <select
                      name="mechanism_of_injury" value={mechanism_of_injury}
                      onChange={handleMoIChange} id="moi"
                    ...
                    {fieldErrors.mechanism_of_injury && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.mechanism_of_injury}</p>}
                  </div>
                  ...
                  {mechanism_of_injury === "RTA injury" && (
                    <div className="flex flex-col">
                      ...
                      <select
                        name="type_of_injury" value={type_of_injury}
                        onChange={handleSubMoIChange} id="subMoi"
                      ...
                      {fieldErrors.type_of_injury && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.type_of_injury}</p>}
                    </div>
                  )}
                  ...
                  {mechanism_of_injury === "fall injury" && (
                    <div className="flex flex-col">
                      <label htmlFor="subMoiFall" className="text-sm font-semibold text-gray-700 mb-2">
                        Fall Injury Type <span className="text-red-500">*</span>
                      </label>
                      <select
                        name="type_of_injury" value={type_of_injury}
                        onChange={handleSubMoIChange} id="subMoiFall"
                        className={`p-3 border ${fieldErrors.type_of_injury ? "border-red-300 bg-red-50" : "border-gray-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all`}
                      >
                  ...
                  {mechanism_of_injury === "sports injury" && (
                    <>
                      <div className="flex flex-col">
                        <label htmlFor="subMoiSports" className="text-sm font-semibold text-gray-700 mb-2">
                          Sports Type <span className="text-red-500">*</span>
                        </label>
                        <select
                          name="subMoi" value={type_of_injury}
                          onChange={handleSubMoIChange} id="subMoiSports"
                          className={`p-3 border ${fieldErrors.type_of_injury ? "border-red-300 bg-red-50" : "border-gray-300"} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all`}
                        >
                      ...
                      {type_of_injury === "others" && (
                        <div className="flex flex-col">
                          ...
                          <input
                            type="text" value={sportsOther}
                            onChange={(e) => setSportsOther(e.target.value)}
                            id="sportsOther" name="sportsOther"
                  ...
                  {mechanism_of_injury === "others" && (
                    <div className="flex flex-col">
                      ...
                      <input
                        type="text" value={MoIOther}
                        onChange={(e) => setMoIOther(e.target.value)}
                        id="MoIOther" name="MoIOther"
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="date" id="incidentDate" name="incident_datetime" value={incidentDate}
                    ...
                    />
                    {fieldErrors.incidentDate && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.incidentDate}</p>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="time" id="incidentTime" name="incident_datetime" value={incidentTime}
                    ...
                    />
                    {fieldErrors.incidentTime && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.incidentTime}</p>}
                  </div>

                  {/* Presentation Delay */}
                  {incidentDate && incidentTime && (
                    <div className="flex flex-col md:col-span-2">
                      <label className="text-sm font-semibold text-gray-700 mb-2">Presentation Delay</label>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <select
                      id="primaryTreatment" name="primary_treatment_received" value={primaryTreatment}
                    ...
                    {fieldErrors.primaryTreatment && <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1"><ErrorIcon />{fieldErrors.primaryTreatment}</p>}
                  </div>
                  ...
                  {primaryTreatment === "1" && (
                    <>
                      <div className="flex flex-col">
                        <label htmlFor="treatmentWhere" className="text-sm font-semibold text-gray-700 mb-2">Treatment Location</label>
                        <input
                          type="text" id="treatmentWhere" value={treatmentWhere}
                      ...
                      <div className="flex flex-col">
                        <label htmlFor="antibioticReceived" className="text-sm font-semibold text-gray-700 mb-2">Antibiotic Received</label>
                        <select
                          id="antibioticReceived" value={antibioticReceived}
                      ...
                      {antibioticReceived === "Yes" && (
                        <div className="flex flex-col">
                          <label htmlFor="antibiotic" className="text-sm font-semibold text-gray-700 mb-2">Antibiotic</label>
                          <input
                            type="text" id="antibiotic" value={antibiotic}
                      ...
                      <div className="flex flex-col md:col-span-2">
                        <label htmlFor="whatTreatment" className="text-sm font-semibold text-gray-700 mb-2">Treatment Details</label>
                        <textarea
                          id="whatTreatment" rows="4" value={whatTreatment}
